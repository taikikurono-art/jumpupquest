...
function renderRankingHot(){
  ...
  el.innerHTML = scored.map((s, i) => {
    ...
    const spriteHtml = getIconDisplay(s.char, 48, false, false);
...
function renderRankingTop(){
  ...
  el.innerHTML = scored.map((s, i) => {
    ...
    const spriteHtml = getIconDisplay(s.char, 40, false, false);
...
function renderFeatured(data){
  ...
  el.innerHTML = data.members.map(m => {
    ...
    if(!char) return '';
    const spriteHtml = getIconDisplay(char, 56, false, false);
